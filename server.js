...
async function initialUserDataPull(req, res) {
  ...
  await superagent.get("https://api.spotify.com/v1/me/top/artists?limit=1&offset=0")
    ...
    .then(data => {
      const sqlString = 'INSERT INTO app_users(fave_artist, first_name, last_name, spotify_user_id) VALUES($1, $2, $3, $4) ON CONFLICT(spotify_user_id) DO UPDATE SET fave_artist=EXCLUDED.fave_artist RETURNING id;';
      const sqlArray = [
